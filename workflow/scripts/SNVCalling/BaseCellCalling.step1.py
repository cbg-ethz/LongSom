			...
			if line.startswith('##'):
				...
				if line.startswith('#CHROM') and counter == 0:
					...
					DICT = {'ALT': "##INFO=ALT,Description=Alternative alleles found",
						...
						'MCF':"##INFO=MCF,Description=Cancer cell fraction (fraction of ditinct cells) supporting the alternative allele in the cell type with the mutation",
					...
					# Add calling columns
					INFO = ['ALT', 'FILTER', 'Cell_types','Up_context', 'Down_context','N_ALT', 'Dp', 'Nc', 'Bc', 'Cc', 'VAF', 'MCF', 'BCp', 'CCp', 'Cell_types_min_BC', 'Cell_types_min_CC', "Rest_BC", "Rest_CC",'Fisher_p', 'Cell_type_Filter']
						...
						if INFO_i.startswith('NA'):
							...
							if (int(DP) >=  min_reads and int(NC) >= min_cells):
								...
								if (len(Alt_candidates) > 0):
									...
									mcf = "|".join([str(round(Alt_cc_dict[x]/float(NC),4)) for x in Alt_candidates])
									MCF.append(mcf)
					...
					if (len(Alts) > 0):
						...
						INFO = [Alts, FILTER, Cell_types, up_context, down_context, str(LEN_Alts), DPs, NCs, BCs, CCs, VAF, MCF, BCp, CCp, Cell_types_min_BC, Cell_types_min_CC, rest_BC, rest_CC, Fisher_p, Filter]
						CALLING_INFO = "\t".join(INFO)
						elements.insert(4,CALLING_INFO)
						outfile.write('\t'.join(elements)+'\n')
					else:
						...
						# Create out line with call performed
						INFO = [Alts, FILTER, Cell_types, up_context, down_context, str(LEN_Alts), DPs, NCs, BCs, CCs, VAF, MCF, BCp, CCp, Cell_types_min_BC, Cell_types_min_BC, rest_BC, rest_CC,Fisher_p, Filter]
						CALLING_INFO = "\t".join(INFO)
						elements.insert(4,CALLING_INFO)
						outfile.write('\t'.join(elements)+'\n')
