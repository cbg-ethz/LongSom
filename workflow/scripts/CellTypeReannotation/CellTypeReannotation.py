...
def write_reannotated_cell_types(cancer_cells,BC_COV_min,bc_file,out_file):
	...
	bcs['Before_Reannotation_cell_type'] = bcs['Cell_type']

    # Cell reannotation based on HCCVs
	bcs['Reannotated_cell_type'] = ['Cancer' if i in cancer_cells else 'Non-Cancer' for i in bcs['Index']]
	
    # Copy reannotated celltypes for SplitBamCellTypes.py compatibility
	bcs['Cell_type'] = bcs['Reannotated_cell_type']

	#bcs['Cancer_Color'] = bcs['Reannotated_cell_type'].apply(lambda x: '#94C773' if x=='Non-Cancer' else '#8F79A1')
