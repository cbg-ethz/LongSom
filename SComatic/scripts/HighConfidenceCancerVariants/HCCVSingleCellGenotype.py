		...
		for bc in CELLS[POS].keys():
			...
			if DP > 0:
				if ALT > 0:
					...
					MUTATED = 'NoAltReads'
			Ref_exp,Alt_exp,Cell_type_exp,Num_cells_exp = Target_sites[POS]
			Group = [str(CHROM), str(POS+1),str(POS+1),Ref_exp,Alt_exp,str(Cell_type_exp),str(Num_cells_exp),bc,CTYPE,str(DP),str(ALT),str(VAF),str(BETABIN),str(MUTATED)]
			Group = '\t'.join(Group) + '\n'
			outfile.write(Group)
...
def concatenate_sort_temp_files_and_write(out_file, tmp_dir):
	...
	if (len(all_files) > 0):
		...
		out = open(out_file,'w')
		Header=['#CHROM','Start', 'End', 'REF', 'ALT_expected', 'Cell_type_expected','Num_cells_expected','CB','Cell_type_observed','Dp','ALT','VAF','BetaBin','MutationStatus']
